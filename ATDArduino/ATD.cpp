...
/* Accepts a single input and outputs whether or not the output added cutCellIndex + 1
 *      data points ago was found.
 */
...
/* Accepts arrays that should all be of length >= length. */
...
/* This resets the ATD, but not the individual parameters. */
void ATD::reset() {
    for (int i = 0; i < _nCells; i++) {
        _window[i] = 0;
    }
...
/* This resets the ATD and also the individual parameters. */
...
/* Returns the element at the cut index */
int ATD::getCut() {
    return _window[_cutCellIndex];
}

/* Returns false until you insert cutCellIndex data. */
bool ATD::isReady() {
    return !(_ready);
}

/* Returns true if the element at the cutCellIndex is a target. */
bool ATD::isFound() {
    return _found;
}

/* Calculates the threshold if the noise is Rayleigh. */
...
/* Returns the maximum value of an array from start to end. */
